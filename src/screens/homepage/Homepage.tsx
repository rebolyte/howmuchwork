...
import { DocumentTitle, useOnMount } from '@utilities';
...
const sumVals = (obj: StringNum) => sum(Object.values(obj));
...
const genExpensesInDays = (periodLength: number, amountMade: number, expenses: StringNum) => {
	const expensesInDays: StringNum = Object.entries(expenses).reduce((acc: StringNum, [k, v]) => {
		const percent = v / amountMade;
		acc[k] = periodLength * percent;
		return acc;
	}, {});

	console.log(expensesInDays);

	const days = [...Array(periodLength)]
		.map(_ => ({}))
		.map((day: StringNum, _i) => {
			while (sumVals(day) < 1) {
